...
#include <complex>
#include <atomic>
typedef uint8_t uint8; typedef int64_t int64; const float c_pi = 3.14159265359f;
// settings
const size_t    c_imageSize_hang = 98; const size_t    c_imageSize_lie = 54; const bool      c_doDFT = true;
const float     c_blurThresholdPercent = 0.00001f; // lower numbers give higher quality results, but take longer. This is 0.5%
const float     c_numBlurs = 5;
char name[]= "D:\\study\\GHOST\\deeplearning_ghostimage\\cpppinknoise\\pink98x54\\";
char* fileName = name;
//======================================================================================
// width, height and pitch uint8
struct SImageData
{
    SImageData()
        : m_width(0)
...
//======================================================================================
// width, height and pitch complex<float>
...
void GetMagnitudeData(const SImageDataComplex& srcImage, SImageData& destImage)
{
    ...
    if (maxmag == 0.0f)
        maxmag = 1.0f;
    const float c = 255.0f / log(1.0f + maxmag);// normalize the magnitude data and send it back in [0, 255]
    for (size_t x = 0; x < srcImage.m_width; ++x)
    {
        for (size_t y = 0; y < srcImage.m_height; ++y)
        {
...
bool ImageSave(const SImageData& image, const char* fileName)
{
    ...
    header.bfSize = infoHeader.biSizeImage + header.bfOffBits;// write the data and close the file
...
{// open the file if we can
    ...
    if (!file)
        return false;// read the headers if we can
    BITMAPFILEHEADER header;
    BITMAPINFOHEADER infoHeader;
...
// 1 + 2 * sqrt(-2 * sigma^2 * ln(c_blurThresholdPercent) )
...
//======================================================================================
// 1/6 * (G(x - 0.5) + 4*G(x) + G(x+0.5))
// x = from -taps/2 to taps/2 in step of 1
...
{
    printf("\n%s\n", fileName);// init the image
    SImageData image;
    ImageInit(image, imageFloat.m_width, imageFloat.m_height);// write the data to the image
...
void NormalizeHistogram(SImageDataFloat& image)
{
    ...
    pixels.resize(image.m_width * image.m_height);// put all the pixels into the array
    ...
    std::shuffle(pixels.begin(), pixels.end(), rng);// sort the pixels by value
    std::sort(pixels.begin(),
        pixels.end(),
        [](const SHistogramHelper& a, const SHistogramHelper& b)
        {return a.value < b.value; });// use the pixel's place in the array as the new value, and write it back to the image
    for (size_t i = 0, c = image.m_width * image.m_height; i < c; ++i)
    {
...
void BlueNoiseTest(float blurSigma)
{
    ...
    ImageFloatInit(noise, c_imageSize_hang, c_imageSize_lie);
    ...
    for (size_t blurIndex = 0; blurIndex < c_numBlurs; ++blurIndex)
    {
        ...
        for (size_t pixelIndex = 0; pixelIndex < c_imageSize_hang * c_imageSize_lie; ++pixelIndex)
...
{// calculate the blur size from our sigma
    int blurSize = PixelsNeededForSigma(blurSigma) | 1;// setup the randon number generator
    std::random_device rd;
    std::mt19937 rng(rd());
    std::uniform_real_distribution<float> dist(0.0f, 1.0f);// generate some white noise
    SImageDataFloat noise;
    ImageFloatInit(noise, c_imageSize_hang, c_imageSize_lie);
    for (float& v : noise.m_pixels)
    {
    ...
    {// get a low passed version of the current image
        ...
        NormalizeHistogram(noise);
        // save this image
        sprintf(fileName, baseFileName, int(blurSigma * 100.0f), blurIndex);
        if (blurIndex == c_numBlurs - 1)
        {
            SaveImageFloatAsBMP(noise, fileName);
        }
...
int main(int argc, char** argv)
{
    ...
    for (k = 1; k < 5293; k++)
    {
        ...
        //    // save this image
        //    sprintf(fileName, baseFileName, int(blurSigma * 100.0f), k);
        //    if (blurIndex == c_numBlurs - 1)
        //    {
        //        SaveImageFloatAsBMP(noise, fileName);
        //    }
